...
source.base64Strs = function () {
  ...
  Object.keys(bufIcons).forEach(function (_) {
    return rtnObj[_] = "data:image/png;base64,".concat(bufIcons[_].toString('base64'));
  });
  return rtnObj;
}();
...
source.loadSingleIcon = function (key) {
  ...
  return new Promise(function (resolve) {
    ...
    image.onload = function () {
      image.onload = null;
      resolve(_defineProperty({}, key, image));
    };

    image.src = _this.base64Strs[key];
  });
};
...
source.loadIcons = function () {
  ...
  return Promise.all(Object.keys(this.base64Strs).map(function (_) {
    return _this2.loadSingleIcon(_);
  })).then(function (values) {
    _this2.isIconsLoaded = true;
    values.forEach(function (item) {
      return _this2.icons = _objectSpread({}, _this2.icons, item);
    });
  });
};
...
function () {
  ...
  _createClass(Block, [{
    ...
    value: function drawText(_ref2) {
      ...
      var tx = this.col * size + (size - space - fw) / 2;
      var ty = this.row * size + (size - space - fw) / 2;
      context.save();
      context.font = "bold ".concat(fontSize, " serif");
    ...
    value: function drawIcon(_ref3) {
      ...
      var dx = this.col * size + space + (size - space - dw) / 2;
      var dy = this.row * size + space + (size - space - dh) / 2;
      context.drawImage(icon, dx, dy, dw, dh);
    }
  }, {
    key: "drawBG",
    value: function drawBG(_ref4) {
      var context = _ref4.context,
          size = _ref4.size,
          space = _ref4.space,
          icon = _ref4.icon;
      context.drawImage(icon, 0, 0, icon.width, icon.height, this.col * size + space, this.row * size + space, size - space, size - space);
    }
  }, {
    key: "draw",
    value: function draw(_ref5) {
      var context = _ref5.context,
          ...
          space = _ref5$space === void 0 ? 6 : _ref5$space;
      var params = {
        context: context,
        size: size,
        space: space
      };
      var _source$icons = source.icons,
...
var utils = {
  ...
  genBlocks: function genBlocks(rows, cols) {
    var blocks = [];
...
function () {
  ...
  _createClass(Game, [{
    ...
    value: function updateSize() {
      ...
      var canvasHeight,
          canvasWidth = this.pixRatio * width;
      var blockSize = (canvasWidth - this.blockSpace) / this.cols;
      var maxHeight = this.pixRatio * (window.innerHeight - 100);
    ...
    value: function drawBlocks() {
      ...
      this.blocks.forEach(function (block) {
        block.draw({
          ...
          size: _this3.blockSize,
          space: _this3.blockSpace
        });
      });
    }
  }, {
    ...
    key: "updateMineMap",
    value: function updateMineMap(block) {
      var _this4 = this;

      var numArray = _toConsumableArray(Array(this.mineCount).fill(9)).concat(_toConsumableArray(Array(this.blocks.length - this.mineCount).fill(0)));

      numArray.sort(function () {
        return Math.random() - .5;
      });
      var curIndex = this.blocks.indexOf(block);
      ...
      numArray.forEach(function (num, index) {
        return _this4.blocks[index].num = num;
      });
      this.updateBlocksNum();
    }
  }, {
    key: "getRoundBlocks",
    value: function getRoundBlocks(i) {
      var tl = this.blocks[i - this.cols - 1];
      var tm = this.blocks[i - this.cols];
      var tr = this.blocks[i - this.cols + 1];
      var r = this.blocks[i + 1];
      var br = this.blocks[i + this.cols + 1];
      var bm = this.blocks[i + this.cols];
      var bl = this.blocks[i + this.cols - 1];
      var l = this.blocks[i - 1];
      var arr = [tl, tm, tr, r, br, bm, bl, l];
    ...
    key: "getTRBLBlocks",
    value: function getTRBLBlocks(i) {
      var t = this.blocks[i - this.cols];
      var r = this.blocks[i + 1];
      var b = this.blocks[i + this.cols];
      var l = this.blocks[i - 1];
      var arr = [t, r, b, l];
    ...
    key: "updateBlocksNum",
    value: function updateBlocksNum() {
      for (var i = 0, len = this.blocks.length; i < len; i++) {
        var block = this.blocks[i];
        if (block.num === 9) continue;
        block.num = this.getRoundBlocks(i).filter(function (_) {
          return _.num === 9;
        }).length;
      }
    }
    ...
    key: "openZeroBlocks",
    value: function openZeroBlocks(block) {
      var checkedBlocks = [],
          noCheckBlocks = [block];
      ...
      while (noCheckBlocks.length) {
        ...
        var roundBlocks = this.getTRBLBlocks(index);

        for (var i = 0, len = roundBlocks.length; i < len; i++) {
          var _ = roundBlocks[i];
